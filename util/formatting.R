...
convertGenes <- function(data) {
  ...
  #  mutate(gene.name = str_extract(Name, "^[^\\*]*")) %>%
    # Group by SimpleName to filter unique names
  #  group_by(gene.name) %>%
  #  filter(n() == 1) %>%
  #  ungroup() %>%
  #  pull(gene.name)
...
# Common use is to identify epitopes with a count above a threshold, and join with epitope information from the reference dataset (optional)
fetchEpitopes <- function(data, column, threshold, reference = TRUE) {
  epitopes <- data %>% 
